...
class Wrapper:
    def __init__(self):
        ...
        local = True 
        models_path = os.path.join(ASSETS_DIR, model_name)
        yolo_path = models_path

        dcss_models_path = "courses/mooc/objdet/data/nn_models/"

        # Replace '/path/to/catkin_ws' with the actual path to your catkin_ws directory
        catkin_ws_path = '/code'
        ...
        if local_etag != remote_etag:
            ...
            print(f"Model with ETag '{remote_etag}' downloaded! at dir {weight_file_path}")
        else:
            print(f"Local model is up-to-date!")

        # # print contents of catkin_ws directory
        # if os.path.exists(catkin_ws_path) and os.path.isdir(catkin_ws_path):
        #     print(f"Contents of catkin_ws directory: {catkin_ws_path}")
        #     self.print_directory_contents(catkin_ws_path)
        # else:
        #     print(f"catkin_ws directory not found at: {catkin_ws_path}")
            
        # load pytorch model
        self.model = Model(weight_file_path, yolo_path)
    ...
    def print_directory_contents(self, path):
        for child in os.listdir(path):
            ...
            if os.path.isdir(child_path):
                ...
                self.print_directory_contents(child_path)  # Recursively print contents of subdirectory
            else:
                print(f"File: {child_path}")

class Model:
    def __init__(self, weight_file_path: str, models_path: str):
        super().__init__()
        model = torch.hub.load(models_path, "custom", path=weight_file_path, source="local")
        model.eval()
